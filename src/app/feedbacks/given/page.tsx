...
export default function GivenFeedbackPage() {
  ...
  const [editAnonymous, setEditAnonymous] = useState<boolean>(false);
  ...
  const handleEdit = (feedback: Feedback) => {
    ...
    setEditAnonymous(false); // Default to non-anonymous
  };
            ...
            <tbody>
              {feedbacks.map((feedback) => (
                <tr key={feedback.id} className="border-b hover:bg-gray-50">
                  <td className="px-4 py-2">{new Date(feedback.created_at).toLocaleDateString()}</td>
