// src/app/instructor/courses/[courseId]/members/page.tsx
...
export default function ManageCourseMembersPage() {
  ...
  useEffect(() => {
    ...
    if (!session || !session.user.isInstructor) router.push('/auth/login');
  ...
  useEffect(() => {
    const fetchCourseData = async () => {
      try {
        ...
        const membersRes = await fetch(`/api/instructor/courses/${courseId}/members`);
  ...
  const debouncedSearch = useCallback(
    debounce(async (query: string) => {
      ...
      try {
        const res = await fetch(`/api/instructor/users?search=${encodeURIComponent(query)}`);
  ...
  const handleAddMember = async (userId: number) => {
    if (!userId) return;
    try {
      const res = await fetch(`/api/instructor/courses/${courseId}/members`, {
        method: 'POST',
  ...
  const handleRemoveMember = async (userId: number) => {
    ...
    try {
      const res = await fetch(`/api/instructor/courses/${courseId}/members/${userId}`, {
        method: 'DELETE',
      });
